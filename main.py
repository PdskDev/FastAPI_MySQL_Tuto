...
@app.get("/posts/{post_id}", status_code=status.HTTP_200_OK, tags=["Get"])
...
@app.get("/posts/", status_code=status.HTTP_200_OK, tags=["Get"])
async def get_posts_list(db: db_dependency):
...
async def create_new_post(post: PostBase, db: db_dependency):
    db_post = models.Post(**post.dict())
    db.add(db_post)
    db.commit()


@app.put("/posts/{post_id}", status_code=status.HTTP_200_OK, tags=["Put"])
...
@app.delete("/posts/{post_id}", status_code=status.HTTP_204_NO_CONTENT, tags=["Delete"])
...
@app.get("/users/{user_id}", status_code=status.HTTP_200_OK, tags=["Get"])
...
@app.get("/users/", status_code=status.HTTP_200_OK, tags=["Get"])
async def get_users_list(db: db_dependency):
...
@app.post("/users/", status_code=status.HTTP_201_CREATED, tags=["Post"])
...
@app.put("/users/{user_id}", status_code=status.HTTP_200_OK, tags=["Put"])
async def update_user(user_id:int, user_to_update: UserBase, db: db_dependency):
...
@app.delete("/users/{user_id}", status_code=status.HTTP_204_NO_CONTENT, tags=["Delete"])
